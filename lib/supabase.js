...
export async function adicionarFollowUp(telegramId, followUpData) {
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      status: 'A Realizar', // Status inicial padrão
      created_at: new Date()
    }]);
...
// Listar follow-ups com filtros
export async function listarFollowUps(telegramId, filtro = {}) {
  let query = supabase
    .from('followups')
    .select('*, clientes(nome_empresa)')
    .eq('telegram_id', telegramId);
...
export async function atualizarStatusFollowUp(followUpId, novoStatus) {
  const { data, error } = await supabase
    .from('followups')
    .update({ 
      status: novoStatus,
...
export async function buscarFollowUpPorId(telegramId, followUpId) {
  const { data, error } = await supabase
    .from('followups')
    .select('*, clientes(nome_empresa)')
    .eq('telegram_id', telegramId)
...
// Funções de lembrete atualizadas
...
// Listar lembretes para o usuário
...
// Buscar lembretes para notificação
export async function buscarLembretesParaNotificar() {
  try {
    // Obter data e hora atual com ajuste para o fuso horário brasileiro (-3)
    const agora = new Date();
    
    // Log detalhado
    console.log('Verificando lembretes para horário:', agora.toISOString());
    
    // Formatar data para o padrão do banco (YYYY-MM-DD)
    const dataAtual = agora.toISOString().split('T')[0];
    
    // Formatar hora (HH:MM)
    const hora = agora.getUTCHours() - 3; // Ajuste para GMT-3 (Brasil)
    const horaFormatada = (hora < 0 ? hora + 24 : hora).toString().padStart(2, '0');
    const minutos = agora.getUTCMinutes().toString().padStart(2, '0');
    const horaAtual = `${horaFormatada}:${minutos}`;
    
    // Incluir log explícito dos valores pesquisados
    console.log(`🔍 Buscando lembretes para: data=${dataAtual}, hora=${horaAtual}`);

    // Consultar lembretes pendentes para a hora atual
    const { data, error } = await supabase
      .from('lembretes')
      .select('*, clientes(nome_empresa)')
      .eq('data', dataAtual)
    ...
    // Registrar todos os lembretes para depuração
    console.log(`✅ Encontrados: ${data?.length || 0} lembretes para notificação`);
    
    if (data && data.length > 0) {
      console.log("📝 Lembretes encontrados:", 
        data.map(l => ({
          id: l.id,
          data: l.data,
          horario: l.horario,
          texto: l.texto.substring(0, 30) + (l.texto.length > 30 ? "..." : "")
        }))
      );
    }
    ...
    console.error("❌ ERRO CRÍTICO na busca de lembretes:", err);
    // Retornar array vazio para não quebrar o fluxo
    return [];
  }
}

export async function buscarLembretesParaNotificar(data, hora) {
  try {
    const { data: lembretes, error } = await supabase
      .from('lembretes')
      .select('*, clientes(nome_empresa)')
      .eq('data', data)
      .eq('horario', hora) // Compara exatamente com HHMM
      .eq('status', 'pendente');
    ...
    console.log(`✅ Encontrados: ${lembretes?.length || 0} lembretes para notificação às ${hora}`);
    return lembretes || [];
  } catch (err) {
    console.error("❌ ERRO CRÍTICO na busca de lembretes:", err);
    return [];
...
export async function atualizarStatusLembrete(lembreteId, status) {
  const { error } = await supabase
    .from('lembretes')
