...
export async function handleClientesListar(ctx: Context) {
  ...
  try {
    ...
    await ctx.editMessageText('❌ Erro ao listar clientes. Tente novamente.');
  }
}
...
export async function handleClientesBuscar(ctx: Context) {
  ...
  // ✅ ADICIONAR: Loading state
  const loadingMsg = await ctx.reply('⏳ Preparando busca...');
  ...
  try {
    ...
    await ctx.editMessageText('❌ Erro ao iniciar busca. Tente novamente.');
  }
}